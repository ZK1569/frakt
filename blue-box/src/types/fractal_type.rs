...
use super::{desc::Range, protocols::FragmentTask};
...
impl CalcFractal for NewtonRaphsonZ3 {
    fn determine_pixel_intensity(&self, x: f64, y: f64, max_iteration: &u32) -> (f32, f32) {
        ...
        loop {
            ...
            if (zn_next - z).norm() < delta || i >= *max_iteration {
                break;
            }
            z = zn_next;
            i += 1;
        ...
        let count = if i < *max_iteration {
            Complex::convergence_value(z.norm() as f32, delta, i, *max_iteration)
        } else {
            1.0
        };
...
impl CalcFractal for NewtonRaphsonZ4 {
    fn determine_pixel_intensity(&self, x: f64, y: f64, max_iteration: &u32) -> (f32, f32) {
        ...
        let zn = z.angle();
        let count = if i < *max_iteration {
            Complex::convergence_value(z.norm() as f32, epsilon, i, *max_iteration)
        } else {
            1.0
        };
